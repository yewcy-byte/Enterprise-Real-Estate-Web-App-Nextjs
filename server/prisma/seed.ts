...
async function resetSequence(modelName: string) {
  ...
  try {
    await pool.query(
      `SELECT setval(pg_get_serial_sequence('\"${toPascalCase(modelName)}\"', 'id'), COALESCE(MAX(id)+1, 1), false) FROM ${quotedModelName};`
    );
...
async function insertGeneric(modelName: string, items: any[]) {
